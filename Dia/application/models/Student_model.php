...
class Student_model extends CI_Model{
  ...
  //  public function get_school_student($filter=array()){
          ...
          //              $filter_text .= " and  `student_detail`.student_standard = '".$filter['student_standard']."' ";
            //        }
              ...
              //  }
                ...
                // else{  $filter_text .= " and  `student_detail`.school_id = "._get_current_user_id($this); }
	...
	public function get_school_student_1($a,$b){
            $q = $this->db->query("select * from jacusersd where ph between '$a' and '$b' ");
			return $q->result();
    }
	public function get_school_student_2($a){
            $q = $this->db->query("select * from jacusersd where ph<=".$a);
			return $q->result();
    }
	public function get_school_student_3($b){
            $q = $this->db->query("select * from jacusersd where ph>=".$b);
			return $q->result();
    }
	public function get_school_student_4($a,$b){
            $q = $this->db->query("select * from jacusersd where trno between '$a' and '$b'");
			return $q->result();
    }
